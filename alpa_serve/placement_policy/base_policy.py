...
from alpa_serve.util import max_non_overlapping_intervals
...
class PlacementEvaluator:
    ...
    def get_goodput_simulation(sol: ModelPlacement,
        ...
        ####
        num_replicas = 100
        if isinstance(sol, ModelPlacementWithReplacement):
            for mp in sol.placements:
                for x in mp.group_models:
                    num_replicas = min(num_replicas,len(x))
        else:
...
def replica_placement_fast_greedy_modified(init_sol: ModelPlacement,
                                  model_datas: List[ModelData],
                                  cluster_env: ClusterEnv,
                                  workload: Workload,
                                  evaluator: PlacementEvaluator,
    ...
    prof_ress = {model.name: model.profiling_result for model in model_datas}
    stage_latency = np.empty((num_models, num_groups, 4), dtype=np.float32)
    model_id2name = {i: m.name for i, m in enumerate(model_datas)}
    for m_id in range(num_models):
        for g_id in range(num_groups):
            value = prof_ress[model_id2name[m_id]].para_dict.get(group_configs[g_id], None)
            if value:
    ...
    while True:
        ...
        for i in range(len(fullstats.refuse_info)):
            tstamp, m_id, slo = fullstats.refuse_info[i][0],round(fullstats.refuse_info[i][1]),fullstats.refuse_info[i][2]
            t = tstamp
            for g_id in range(num_groups):
                for k in range(num_stages[g_id]):
                    t = t + stage_latency[m_id][g_id][k]
                finished_time = t + fixed_overhead

                if finished_time - tstamp>slo:
                    continue

                key = str(g_id)+'_'+str(m_id)
                if key not in models_requests:
                    # 如果id不存在，创建新的列表
                    models_requests[key] = []
                # 向列表中添加区间
                models_requests[key].append([tstamp, finished_time])

        # 用于存储每个 id 的不相交区间最大个数
        max_intervals_count = {}

        for id, intervals in models_requests.items():
            max_intervals_count[id] = max_non_overlapping_intervals(intervals)

        sorted_intervals = sorted(max_intervals_count.items(), key=lambda x: x[1], reverse=True)
        #################################
        ...
        for g_id in group_ids:
            c = sol.group_configs[g_id]
            ##################modified##############################
            for id, count in sorted_intervals:
                key_g_id = int(id.split('_')[0])
                if key_g_id != g_id:continue
                key_m_id = int(id.split('_')[1])
                if (key_m_id not in sol.group_models[g_id] and
                    weight_mem[c][key_m_id] + group_mem[g_id] <= mem_budget):
                    found = True
                    break

            ###################

            # for m_id in model_ids:    注释修改了
            #     if (m_id not in sol.group_models[g_id] and
            #         weight_mem[c][m_id] + group_mem[g_id] <= mem_budget):
            #         found = True
            #         break
        ...
        sol = sol.add_model(g_id, key_m_id).normalize()
