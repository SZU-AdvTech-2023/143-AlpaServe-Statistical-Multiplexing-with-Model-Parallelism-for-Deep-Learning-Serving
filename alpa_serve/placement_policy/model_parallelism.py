...
from alpa_serve.placement_policy.base_policy import (
    ...
    replica_placement_round_robin,replica_placement_fast_greedy_modified,
...
def solve_separation_placement(self,
    ...
    for i, eco in enumerate(eco_separation):
        ...
        # eco_sol, _ = self.solve_placement_one_eco_modified(sub_model_datas, sub_cluster_env, train_workload)
...
class ModelParallelismSearch(BasePlacementPolicy):
    ...
    def are_model_placements_equal(self,mp1, mp2):
        # 检查 group_configs 和 group_models 的长度
        if len(mp1.group_configs) != len(mp2.group_configs) or \
        len(mp1.group_models) != len(mp2.group_models):
            return False

        # 用于标记已经匹配过的配置
        matched_indices = set()

        for i, config1 in enumerate(mp1.group_configs):
            found_match = False
            for j, config2 in enumerate(mp2.group_configs):
                if j in matched_indices:
                    continue  # 跳过已经匹配过的配置
                if config1 == config2 and mp1.group_models[i] == mp2.group_models[j]:
    ...
    def model_placement_to_dict(self,model_placement):
        model_configs_dict = {}

        for config, models in zip(model_placement.group_configs, model_placement.group_models):
            for model in models:
                if model not in model_configs_dict:
                    model_configs_dict[model] = [config]
                else:
                    model_configs_dict[model].append(config)

        return model_configs_dict

    def remove_matching_configs_one_by_one(self,dict_before, dict_cur):
        for model, cur_configs in list(dict_cur.items()):
            if model in dict_before:
                before_configs = dict_before[model]

                # 逐个移除匹配的配置元素
                for config in list(cur_configs):
                    if config in before_configs:
                        cur_configs.remove(config)
                        before_configs.remove(config)

                # 更新或删除空的配置列表
                if not cur_configs:
                    del dict_cur[model]
                else:
                    dict_cur[model] = cur_configs

        return dict_cur

    def get_max_latency(self,dict_cur,model_datas):
        ###
        # config_models_dict = {}
        # for model,cur_configs in list(dict_cur.items()):
        #     for cur_config in cur_configs:
        #         if cur_config not in config_models_dict:
        #             config_models_dict[cur_config] = [model]
        #         else:
        #             config_models_dict[cur_config].append(model)
        ###

        max_size = 0
        for model, cur_configs in list(dict_cur.items()):
            for cur_config in cur_configs:
                m_size = max(model_datas[model].profiling_result.para_dict[cur_config].weight_mem)
                max_size = max(max_size,m_size)
        
        # for config,models in list(config_models_dict.items()):
        #     g_m_size = 0
        #     for model in models:
        #         g_m_size += max(model_datas[model].profiling_result.para_dict[config].weight_mem)
        #     max_size = max(max_size,g_m_size)

        return max_size/1024/1024/1024/4
    
    def final_version(self,sol_before,sol_cur,model_datas):
        max_size = 0
        model_ids = []
        for config_cur, models_cur in zip(sol_cur.group_configs, sol_cur.group_models):
            left_models = copy.deepcopy(models_cur)
            for config_before,models_before in zip(sol_before.group_configs,sol_before.group_models):
                if config_cur == config_before:
                    left_models = [x for x in models_cur if x not in models_before]
            g_m_size = 0
            for model in left_models:
                model_ids.append(model)
                g_m_size += max(model_datas[model].profiling_result.para_dict[config_cur].weight_mem)
            # groups_latency.append(g_m_size/1024/1024/1024/4)
            max_size = max(max_size,g_m_size)
        return max_size*1.1/1024/1024/1024/4,model_ids
    
    def exclude_request_goodput(self,sol_before: ModelPlacement,sol_cur: ModelPlacement,model_datas: List[ModelData]):
        if self.are_model_placements_equal(sol_before,sol_cur):
            return 0,[]
        else:
            #找到调度后方案组内增加的模型
            # model_configs_dict_before = self.model_placement_to_dict(sol_before)
            # model_configs_dict_cur = self.model_placement_to_dict(sol_cur)
            # dict_cur = self.remove_matching_configs_one_by_one(model_configs_dict_before,model_configs_dict_cur)
            # max_latency = self.get_max_latency(dict_cur,model_datas)*1.1

            max_latency,model_ids = self.final_version(sol_before,sol_cur,model_datas)
            # pt = 0
            # for arrival in workload.arrivals:
            #     if arrival>=end_time:
            #         break
            #     pt += 1
            # workload.arrivals = workload.arrivals[pt:]
            return max_latency,model_ids

    ####模型定时调度修改代码
    def solve_placement_one_eco_modified(self,
                                model_datas: List[ModelData],
                                cluster_env: ClusterEnv,
                                train_workload: Workload = None):
        # evaluator = PlacementEvaluator(model_datas, cluster_env, train_workload,     modified
        #     self.evaluator_method, self.parallel_evaluator)

        ws = train_workload.split_time_interval(self.replacement_interval)  #append
        start_times = []
        placements = []
        ...
        if self.parallel_initial_placement:
            ...
            for i in range(len(ws)):
                evaluator = PlacementEvaluator(model_datas, cluster_env, ws[i],
                                           "fast_simulator", False)
                initial_sols = self.enumerate_group_configs_uneven(cluster_env)
                for j in range(len(initial_sols)):
                    initial_sols[i] = func(
                    initial_sols[i], model_datas, cluster_env, ws[i], None,
                    self.verbose)
                initial_sols = ray.get(initial_sols)
                scores = evaluator.get_scores(initial_sols)
                best_idx = np.argmax(scores)
                ...
                model_ids = []
                if len(placements)>1:
                    max_latency,model_ids = self.exclude_request_goodput(placements[-1],best_sol,model_datas)
                    #获取模型差异,先获取score大于上一阶段方案score的所有方案，再对这些方案进行筛选
                    # score_before = evaluator.get_scores([placements[-1]])[0]
                    # sol_before = placements[-1]
                    # max_score = 0 
                    # for i in range(len(scores)):
                    #     score_cur = scores[i]
                    #     if score_cur > score_before:
                    #         #获取经历空档期后的score_real
                    #         sol_cur = initial_sols[i]
                    #         copy_ws = copy.deepcopy(ws[i])
                    #         max_latency = self.exclude_request_goodput(sol_before,sol_cur,copy_ws,model_datas)
                    # #删除模型调度空档期的请求(设本服务器pcle加载模型速率为400MB/s，卸载为其100倍，即40G/s)
                    #         evaluator = PlacementEvaluator(model_datas, cluster_env, copy_ws,
                    #                        "fast_simulator", False,[max_latency])
                    # #获取在当前工作负载下，调度模型后能获取的吞吐量
                    #         score = evaluator.get_scores([sol_cur])[0]
                    #         if score>score_before:
                    #             if score>max_score:
                    #                 best_sol = sol_cur
                latencies.append(max_latency)
                models_ids.append(model_ids)
            ...
            for i in range(len(ws)):
                ...
                initial_sols = self.enumerate_group_configs_uneven(cluster_env)
                for j in range(len(initial_sols)):
                    initial_sols[j] = replica_placement_fast_greedy(
                        initial_sols[j], model_datas, cluster_env, ws[i],
                        evaluator, self.verbose)
            ...
            # for i in range(len(initial_sols)):  modified
            #     initial_sols[i] = replica_placement_fast_greedy(
            #         initial_sols[i], model_datas, cluster_env, train_workload, evaluator,
            #         self.verbose)
                #initial_sols[i] = replica_placement_beam_search(
        ...
        return ModelPlacementWithReplacement(start_times, placements,latencies,models_ids), None
    ...
    def solve_placement(self,
        ...
        best_sol, _ = self.solve_placement_one_eco_modified(model_datas, cluster_env, train_workload)
