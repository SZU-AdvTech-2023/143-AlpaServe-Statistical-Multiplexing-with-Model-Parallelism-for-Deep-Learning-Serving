...
def approximate_one_case(case: ServingCase,
    ...
    if isinstance(placement, ModelPlacement):
        ...
        ###
        for i in range(len(arrivals)):
            if placement.latencies and arrivals[i]<float(arrivals[start_i])+float(placement.latencies[pt]) and model_ids[i] in placement.models_ids[pt]:
                mark_idx.append(i-start_i)
            if arrivals[i] > change_times[pt]:
                ...
                max_len = max(max_len,group_num_requests.size)
        ...
        ###################
        for i in range(len(group_num_requests_list)):
            group_num_requests = group_num_requests_list[i]
            sz = group_num_requests.size
            if sz<max_len:
                group_num_requests = np.append(group_num_requests, [0] * (max_len - sz))
                group_num_requests_list[i] = group_num_requests

        for i in range(len(group_num_good_requests_list)):
            group_num_good_requests = group_num_good_requests_list[i]
            sz = group_num_good_requests.size
            if sz<max_len:
                group_num_good_requests = np.append(group_num_good_requests, [0] * (max_len - sz))
                group_num_good_requests_list[i] =  group_num_good_requests
        ###################
...
@numba.jit(nopython=True)
def simulate_requests_mixed_modified(finish, good, tstamps, model_ids, slos, m_id2g_id,
                            num_stages, stage_latency, num_requests):
    ...
    # Arrays to store invalid and timeout requests information
    refuse_info = np.zeros((num_requests, 4))  # timestamp, model_id, slo, finish_time
    refuse_count = 0
    ...
    for i in range(num_requests):
        ...
        if g_id < 0:
            finish[i] = tstamp
            good[i] = False
            # Store information for invalid and timeout requests
            refuse_info[refuse_count] = [tstamp, m_id, slo, -1]
            refuse_count += 1
            continue
    ...
    # Trim the refuse_info array to the actual number of invalid and timeout requests
    refuse_info = refuse_info[:refuse_count]

    return (model_num_requests, model_num_good_requests,
            group_num_requests, group_num_good_requests, refuse_info)
