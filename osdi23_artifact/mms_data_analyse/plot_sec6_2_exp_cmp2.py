...
def plot_goodput_common_cmp(azure_data_before, azurev2_data_after, threshold, increasing, xlabel, output, ybottem, plot_legend=False):
    ...
    for data_before,data_after, ax, title in zip(azure_data_before , azurev2_data_after, axs, titles):
        ...
        for i, method in enumerate(methods):
            ...
            results = []
            for a,b in zip(ys_after,ys_before):
                if b!=0:
                    results.append(a/b)
                else:
                    results.append(1)
            ...
            curve_after = ax.plot(xs, results, color=methodcolors[1], marker='.', linestyle=linestyles[1], linewidth=4,
                            markersize=15)
            ...
            x_max = max(x_max, *xs)
            y_max = max(y_max, *results)

        ax.tick_params(axis='both', which='major', labelsize=20)
        ax.tick_params(axis='both', which='minor', labelsize=20)
        ax.set_ylim(bottom=ybottem, top=max(y_max * 1.02, 1.5))
        ax.grid()
    ...
    fig.text(0.1, 0.5, "rate", va='center', rotation='vertical', fontsize=20)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", type=str, default="paper_figures_cmp2")
    ...
    plot_goodput_vs_num_devices_cmp(azure_lines_before,azure_lines_after,threshold,args.output_dir,args.pdf,0.9,True)

    plot_goodput_vs_rate_scale_cmp(azure_lines_before, azure_lines_after, threshold, args.output_dir, args.pdf, 0.9)
    plot_goodput_vs_cv_scale_cmp(azure_lines_before, azure_lines_after, threshold, args.output_dir, args.pdf, 0.9)
    plot_goodput_vs_slo_cmp(azure_lines_before, azure_lines_after, threshold, args.output_dir, args.pdf, 0.9)
